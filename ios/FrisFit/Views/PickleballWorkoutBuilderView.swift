...
struct PickleballWorkoutBuilderView: View {
    @Bindable var pickleVM: PickleballViewModel
    @Environment(\.dismiss) private var dismiss

    @State private var sessionName: String = ""
    @State private var draftDrills: [PickleballDrillItem] = []
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 18) {
                    ...
                    nameSection
                    drillsSection
                    if !pickleVM.savedSessions.isEmpty {
    ...
    private var nameSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            TextField("e.g. Wednesday Soft Game, Pre-Match Warmup", text: $sessionName)
    ...
    private func drillRow(idx: Int, item: PickleballDrillItem) -> some View {
        HStack(spacing: 12) {
    ...
    private var savedSessionsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 8) {
                ForEach(pickleVM.savedSessions) { session in
                    HStack {
                        VStack(alignment: .leading, spacing: 2) {
                            Text(session.name)
                                .font(.system(size: 13, weight: .semibold, design: .serif))
                        ...
                        Spacer()
                        Button {
                            pickleVM.savedSessions.removeAll { $0.id == session.id }
                        } label: {
        ...
        NavigationStack {
            ScrollView {
                LazyVStack(spacing: 8) {
                    ForEach(PickleballDrillLibrary.all) { drill in
                        Button {
                            draftDrills.append(PickleballDrillItem(drillName: drill.name, durationMinutes: drill.durationMinutes))
    ...
    private func save() {
        ...
        let session = CustomPickleballSession(name: trimmed, drills: draftDrills)
        pickleVM.savedSessions.insert(session, at: 0)
        dismiss()
    }
}
