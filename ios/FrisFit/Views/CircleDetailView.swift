...
struct CircleDetailView: View {
    ...
    private var tabContent: some View {
        ...
        case .leaderboard:
            CircleLeaderboardTab(viewModel: viewModel)
        case .posts:
...
struct CircleLeaderboardTab: View {
    @Bindable var viewModel: CirclesViewModel
    @State private var expandedMember: UUID?
...
struct LeaderboardMemberRow: View {
        ...
        Button {
            ...
            VStack(spacing: 0) {
                HStack(spacing: 12) {
                    ...
                    Text("\(member.totalPoints.formatted())")
                        .font(.system(.subheadline, design: .rounded, weight: .bold))
        ...
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(alignment: .bottom, spacing: 3) {
                ForEach(Array(member.weeklyHistory.enumerated()), id: \.offset) { _, value in
                    let height = max(CGFloat(value) / CGFloat(maxVal) * 40, 3)
