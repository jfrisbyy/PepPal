...
struct RunningDashboardView: View {
    ...
    private var quickStatsHeader: some View {
        EditorialSportHeader(
            kicker: "Running",
            title: "On the Road",
            subtitle: "\(runVM.thisWeekRuns.count) run\(runVM.thisWeekRuns.count == 1 ? "" : "s") this week  ·  \(String(format: "%.1f", runVM.thisWeekMiles)) mi",
            accent: accentColor,
            stats: [
                EditorialStat(String(format: "%.0f", runVM.totalMilesAllTime), "Miles"),
                EditorialStat("\(runVM.totalRunsAllTime)", "Runs"),
                EditorialStat(formatPace(runVM.averagePaceAllTime), "Pace"),
                EditorialStat(String(format: "%.1f", runVM.longestRunEver), "Long")
            ]
        ) {
            Button {
