...
struct TennisDrillDetailView: View {
    let drill: TennisDrill
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            ZStack {
                ...
                GeometryReader { geo in
                    ...
                    Rectangle()
                        .stroke(PepTheme.textPrimary.opacity(0.06), lineWidth: 0.5)
                        .frame(width: geo.size.width * 0.6, height: geo.size.height * 0.55)
    ...
    private func commonMistakes(for category: TennisDrillCategory) -> [String] {
        switch category {
        case .serve:
            return [
                "Tossing inconsistently — anchor the toss with a straight arm.",
                "Hitting flat with no spin margin on second serves.",
                "Stopping the swing early instead of pronating through contact."
            ]
        case .groundstroke:
            return [
                "Late prep — racket back as soon as you read the ball.",
                "Hitting from a stationary stance instead of stepping in.",
                "Decelerating through contact instead of swinging out."
            ]
        case .volley:
            return [
                "Big backswing — short, compact punches only.",
                "Letting the racket head drop below the wrist.",
                "Stopping at contact instead of moving forward through the ball."
            ]
        case .footwork:
            return [
                "Watching the ball from a static stance.",
                "Forgetting the split step on every contact.",
                "Heavy first step — push off the outside foot instead."
            ]
        case .mental:
            return [
                "Skipping the routine when momentum shifts.",
                "Replaying the last point instead of resetting.",
                "Holding the breath through pressure points."
            ]
        case .conditioning:
            return [
                "Pacing the early reps and gassing the last set.",
                "Skipping the cool-down and stiffening up.",
                "Letting the form fall apart when fatigued."
            ]
        }
    }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 8) {
                progressionRow(
                    ...
                    color: TennisDrillDifficulty.beginner.color
                )
                progressionRow(
                    label: "Match",
                ...
                progressionRow(
                    ...
                    color: TennisDrillDifficulty.advanced.color
                )
            }
        }
        .editorialCard(accent: drill.category.color)
    ...
    private func progressionUp(for drill: TennisDrill) -> String {
        switch drill.category {
        case .serve: "Add a target zone, lower the second-serve net margin, or hit on the run."
        case .groundstroke: "Live ball with a defender, smaller target windows, or one-touch returns."
        case .volley: "Increase pace, add deflections, or shorten the reaction window."
        case .footwork: "Add a ball, shrink the cone spacing, or shorten rest."
        case .mental: "Run drills with score on the line — every miss is minus one point."
        case .conditioning: "Add a racket between sprints or shorten rest periods."
        }
    }

    private var relatedCard: some View {
        let related = TennisDrillLibrary.all.filter { $0.category == drill.category && $0.id != drill.id }.prefix(4)
