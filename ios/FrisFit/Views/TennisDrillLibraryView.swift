...
struct TennisDrillLibraryView: View {
    @Environment(\.dismiss) private var dismiss
    @State private var searchText: String = ""
    @State private var selectedCategory: TennisDrillCategory? = nil
    @State private var selectedDifficulty: TennisDrillDifficulty? = nil
    @State private var bound: TennisDrill? = nil

    private let accentColor = Color(red: 0.85, green: 0.9, blue: 0.15)

    private var filteredDrills: [TennisDrill] {
        var drills = TennisDrillLibrary.all
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(item: $bound) { drill in
                TennisDrillDetailView(drill: drill)
            }
        }
    }
        ...
        PepSportCard(accent: accentColor) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .firstTextBaseline) {
                    ...
                    Spacer()
                    Text("\(filteredDrills.count) of \(TennisDrillLibrary.all.count)")
                        .font(.system(size: 9, weight: .bold))
                        .tracking(1.4)
                        .foregroundStyle(PepTheme.textSecondary)
                }

                Text("Sharpen the strings.")
                    .font(.system(size: 24, weight: .semibold, design: .serif))
                    .kerning(-0.5)
                    .foregroundStyle(PepTheme.textPrimary)

                Text("Curated drills across \(TennisDrillCategory.allCases.count) skill areas — tap any drill for the full breakdown, coaching cues, and progressions.")
    ...
    private var difficultyFilter: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 8) {
                ...
                ForEach(TennisDrillDifficulty.allCases) { diff in
    ...
    private func drillRow(_ drill: TennisDrill) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .firstTextBaseline) {
                HStack(spacing: 5) {
