...
struct PickleballDrillLibraryView: View {
    @Environment(\.dismiss) private var dismiss
    @State private var searchText: String = ""
    @State private var selectedCategory: PickleballDrillCategory? = nil
    @State private var selectedDifficulty: PickleballDrillDifficulty? = nil
    @State private var selectedDrill: PickleballDrill? = nil

    private let accentColor = Color(red: 0.62, green: 0.86, blue: 0.18)

    private var filteredDrills: [PickleballDrill] {
        var drills = PickleballDrillLibrary.all
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(item: $selectedDrill) { drill in
                PickleballDrillDetailView(drill: drill)
            }
        }
    }
        ...
        PepSportCard(accent: accentColor) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(alignment: .firstTextBaseline) {
                    ...
                    Spacer()
                    Text("\(filteredDrills.count) of \(PickleballDrillLibrary.all.count)")
                        .font(.system(size: 9, weight: .bold))
                        .tracking(1.4)
                        .foregroundStyle(PepTheme.textSecondary)
                }

                Text("Sharpen the soft hands.")
                    .font(.system(size: 24, weight: .semibold, design: .serif))
                    .kerning(-0.5)
                    .foregroundStyle(PepTheme.textPrimary)

                Text("Curated drills across \(PickleballDrillCategory.allCases.count) skill areas — tap any drill for the full breakdown, coaching cues, and progressions.")
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            ScrollView(.horizontal) {
                HStack(spacing: 8) {
                    ...
                    ForEach(PickleballDrillCategory.allCases) { cat in
    ...
    private var difficultyFilter: some View {
        HStack(spacing: 8) {
            ...
            ForEach(PickleballDrillDifficulty.allCases) { diff in
    ...
    private func drillCard(_ drill: PickleballDrill) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 12) {
                ZStack {
                    Circle()
...
struct PickleballDrillDetailView: View {
    let drill: PickleballDrill
    @Environment(\.dismiss) private var dismiss
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack(alignment: .firstTextBaseline) {
                    Text(drill.category.rawValue.uppercased())
                        .font(.system(size: 10, weight: .semibold))
                        .tracking(2.0)
                        .foregroundStyle(drill.category.color.opacity(0.95))
                    Spacer()
                    Text(drill.difficulty.rawValue.uppercased())
                        .font(.system(size: 9, weight: .bold))
                        .tracking(1.4)
